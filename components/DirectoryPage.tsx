...
type Region = 'us' | 'aus' | 'ten' | 'five' | 'nineteen' | 'barber' | 'localbusiness' | 'freewebsite' | 'freewebsite49' | 'home' | 'denmark' | 'localbusiness29' | 'localbusiness9' | 'barberleads' | 'barberFive' | 'barberFiveMonth' | 'barberTrial' | 'barber19' | 'barber19Hosting' | 'barberNine' | 'barberTwentyNine' | 'localbusinessNine' | 'barberNineLeads' | 'localbusinessNineLeads' | 'localbusinessNineSpanish' | 'barberFiveNine';
...
const REGIONS: Record<Region, {
  ...
  denmark: {
    ...
    ribbonLocation: 'Austin · TX',
    phoneHref: 'tel:+18302549274',
    phoneLabel: 'Ring til os · Hjælp 24/7',
    phoneNumber: '(830) 254-9274',
...
const DirectoryPage: React.FC<{ region?: Region }> = ({ region = 'us' }) => {
  ...
  const fallbackToHosted = async () => {
    try {
      const res = await fetch('/api/create-checkout', {
        ...
        body: JSON.stringify({ plan: pricingPlan, source: cfg.source, embedded: false, ...((region === 'home' || region === 'denmark' || region === 'localbusiness29' || region === 'localbusiness9' || region === 'five' || region === 'barberFive' || region === 'barberFiveMonth' || region === 'barberTrial' || region === 'barberNine' || region === 'barberTwentyNine' || region === 'localbusinessNine' || region === 'barberFiveNine' || region === 'localbusinessVoice') ? { tier: activeHomeTier } : {}) }),
      });
  ...
  useEffect(() => {
    if (region !== 'home' && region !== 'denmark' && region !== 'localbusiness29' && region !== 'localbusiness9' && region !== 'five' && region !== 'barberFive' && region !== 'barberFiveMonth' && region !== 'barberTrial' && region !== 'barber19' && region !== 'barber19Hosting') return;
  ...
  const handleCheckout = async (tierOverride?: 'single' | 'multi', planOverride?: 'monthly' | 'yearly') => {
    ...
    const usesTier = region === 'home' || region === 'denmark' || region === 'localbusiness29' || region === 'localbusiness9' || region === 'five' || region === 'barberFive' || region === 'barberFiveMonth' || region === 'barberTrial' || region === 'barber19' || region === 'barberNine' || region === 'barberTwentyNine' || region === 'localbusinessNine' || region === 'barberFiveNine' || region === 'localbusinessVoice';
    ...
    const localbusiness9Monthly = effectiveTier === 'single' ? 9 : 19;
    ...
    const value = region === 'home'
      ...
      : region === 'localbusiness9'
      ? localbusiness9Monthly
  ...
  if (region === 'localbusinessVoice') {
    ...
    // $29/mo single tier with 1-day free trial. Black-and-white glossy
    // premium look with a chrome orb and highlighted Talk-to-Mia card.
    return (
      <>
        <style>{`
          ...
          .lbv-page {
            min-height:100vh;
            background:
              radial-gradient(ellipse 70% 50% at 50% -10%, rgba(255,255,255,.10) 0%, rgba(255,255,255,0) 60%),
          ...
          .lbv-page::before, .lbv-page::after {
            content:''; position:absolute; border-radius:999px; filter:blur(90px); pointer-events:none; z-index:0;
          }
          .lbv-page::before { width:380px; height:380px; top:-100px; right:-80px; background:radial-gradient(circle, rgba(255,255,255,.18) 0%, rgba(255,255,255,0) 70%); }
          ...
          .lbv-stack { display:flex; flex-direction:column; gap:10px; max-width:540px; margin:0 auto; position:relative; z-index:2; }
          .lbv-card {
            position:relative;
            border-radius:20px;
            padding:16px 16px;
          ...
          .lbv-title {
            font-family:'Playfair Display', Georgia, serif; font-weight:700; font-size:24px; line-height:1.08; letter-spacing:-.018em;
            color:#fafafa; margin:0 0 8px; position:relative;
          }
          .lbv-title em {
          ...
          .lbv-sub {
            ...
            font-size:14px; line-height:1.4; color:#c8c8c8; margin:0 0 12px; max-width:500px; position:relative;
          ...
          .lbv-allin {
            ...
            color:#0a0a0a; font-family:'Playfair Display', Georgia, serif; font-weight:700; font-size:13px; letter-spacing:-.005em;
            box-shadow:
          ...
          .lbv-allin svg { width:14px; height:14px; }
          .lbv-guarantee {
            ...
            font-family:'Inter', sans-serif;
            font-size:11px; font-weight:600; color:#e0e0e0; line-height:1.3; letter-spacing:.01em;
            position:relative;
          }
          .lbv-guarantee span { display:inline-flex; align-items:center; gap:5px; }
          .lbv-guarantee svg { width:12px; height:12px; color:#ffffff; flex-shrink:0; }
          ...
          .lbv-tryit {
            ...
            box-shadow:
              ...
              0 28px 56px -18px rgba(0,0,0,.85),
              0 10px 24px -10px rgba(255,255,255,.07),
              inset 0 1px 0 rgba(255,255,255,.14);
            animation:lbvTryitGlow 4.5s ease-in-out infinite;
          }
          @keyframes lbvTryitGlow {
            0%,100% { box-shadow:0 0 0 1px rgba(255,255,255,.14), 0 0 0 5px rgba(255,255,255,.05), 0 28px 56px -18px rgba(0,0,0,.85), 0 10px 24px -10px rgba(255,255,255,.07), inset 0 1px 0 rgba(255,255,255,.14); }
            50%     { box-shadow:0 0 0 1px rgba(255,255,255,.20), 0 0 0 6px rgba(255,255,255,.08), 0 32px 64px -18px rgba(0,0,0,.9), 0 14px 30px -10px rgba(255,255,255,.11), inset 0 1px 0 rgba(255,255,255,.16); }
          }
          .lbv-tryit::before {
          ...
          .lbv-tryit-tag {
            ...
            color:#0a0a0a; font-family:'Inter', sans-serif; font-size:9px; font-weight:900; letter-spacing:.20em; text-transform:uppercase;
            padding:5px 11px; border-radius:999px;
            box-shadow:0 8px 18px rgba(0,0,0,.55), inset 0 1px 0 rgba(255,255,255,.95), inset 0 -1px 0 rgba(0,0,0,.10);
            white-space:nowrap;
          }
          .lbv-tryit-tag::after { content:'▼'; font-size:7px; color:#0a0a0a; }
          .lbv-tryit-h {
            font-family:'Playfair Display', Georgia, serif; font-weight:700; font-size:18px; line-height:1.15; letter-spacing:-.012em;
            color:#fafafa; text-align:center; margin:4px 0 4px;
          }
          .lbv-tryit-h em {
          ...
          .lbv-tryit-cap {
            font-family:'Cormorant Garamond', Georgia, serif;
            font-size:13px; line-height:1.35; color:#cfcfcf; text-align:center; margin:0 auto 10px; max-width:340px;
            font-weight:400;
          }

          /* VAPI card override — black glossy inside this page */
          .lbv-page .vc-card {
            background:linear-gradient(160deg, #141414 0%, #060606 100%) !important;
          ...
          .lbv-page .vc-card::before {
            background:linear-gradient(135deg, rgba(255,255,255,.85), rgba(255,255,255,.05) 40%, rgba(255,255,255,.55)) !important;
            opacity:.4 !important;
          }
          .lbv-page .vc-badge {
          ...
          .lbv-page .vc-status-live { background:rgba(120,200,160,.16) !important; color:#9be0b4 !important; box-shadow:inset 0 0 0 1px rgba(120,200,160,.4) !important; }
          .lbv-page .vc-wave span { background:linear-gradient(180deg, #ffffff, #a0a0a0) !important; }
          .lbv-page .vc-ctrl {
            background:rgba(255,255,255,.06) !important;
            color:#f4f4f5 !important;
            box-shadow:inset 0 0 0 1px rgba(255,255,255,.14) !important;
          }
          .lbv-page .vc-ctrl:hover { background:rgba(255,255,255,.10) !important; }
          .lbv-page .vc-ctrl-end { background:linear-gradient(180deg, #ff5a7f, #d83a5f) !important; color:#fff !important; box-shadow:0 6px 16px rgba(216,58,95,.45), inset 0 1px 0 rgba(255,255,255,.30) !important; }
          .lbv-page .vc-hint { color:#888 !important; }
          .lbv-page .vc-err { background:rgba(255,140,140,.12) !important; color:#ffb0b0 !important; box-shadow:inset 0 0 0 1px rgba(255,140,140,.35) !important; }

          .lbv-feats { list-style:none; padding:0; margin:4px 0 0; display:flex; flex-direction:column; gap:7px; counter-reset:lbvfeat; }
          .lbv-feats li {
            counter-increment:lbvfeat;
            font-family:'Cormorant Garamond', Georgia, serif;
            font-size:14px; font-weight:500; color:#cfcfcf; line-height:1.35;
            padding:9px 12px 9px 40px; position:relative;
          ...
          .lbv-feats li::before {
            ...
            color:#0a0a0a; font-family:'Playfair Display', Georgia, serif; font-size:11px; font-weight:700; font-style:italic;
            display:inline-flex; align-items:center; justify-content:center; line-height:1;
            box-shadow:0 3px 8px rgba(0,0,0,.5), inset 0 1px 0 rgba(255,255,255,.95);
          }
          .lbv-feats li b { color:#fafafa; font-weight:700; font-family:'Playfair Display', Georgia, serif; font-size:.95em; }

          .lbv-mini-h {
            font-family:'Playfair Display', Georgia, serif;
            font-style:italic; font-weight:600; font-size:17px; letter-spacing:-.005em;
            color:#fafafa;
            margin:0 0 10px;
          }
          ...
          @media (min-width: 760px) {
            .lbv-stack { max-width:600px; }
            .lbv-card { padding:22px 22px; }
            .lbv-title { font-size:34px; }
            .lbv-sub { font-size:16px; }
            .lbv-feats li { font-size:15px; }
            .lbv-tryit-h { font-size:22px; }
            .lbv-tryit-cap { font-size:14px; }
          }

          @keyframes lbvFadeIn { from { opacity:0 } to { opacity:1 } }
          .mv-checkout-backdrop { position:fixed; inset:0; background:rgba(0,0,0,.78); backdrop-filter:blur(8px); -webkit-backdrop-filter:blur(8px); z-index:9999; display:flex; align-items:center; justify-content:center; padding:20px; animation:lbvFadeIn .2s ease; }
          .mv-checkout-modal { position:relative; width:100%; max-width:440px; max-height:calc(100vh - 40px); background:#fff; border-radius:22px; box-shadow:0 30px 70px rgba(0,0,0,.6); padding:8px; overflow:hidden; display:flex; flex-direction:column; }
          .mv-checkout-close { position:absolute; top:8px; right:8px; width:30px; height:30px; border-radius:999px; background:#0a0a0a; border:0; color:#fff; font-size:13px; cursor:pointer; z-index:3; }
          .mv-checkout-close:hover { background:#2a2a2a; }
          .mv-checkout-frame-inner { flex:1; overflow-y:auto; border-radius:14px; }
          .mv-checkout-fallback-link { display:block; margin:8px auto 4px; padding:6px 10px; background:transparent; border:0; cursor:pointer; font-size:11px; letter-spacing:.12em; text-transform:uppercase; color:#888; font-weight:600; text-align:center; }
          .mv-checkout-fallback-link:hover { color:#0a0a0a; }
        `}</style>
...
Keep replies short — 1 to 3 sentences usually. This is a phone call, not an essay.`,
  ...
  if (region === 'barberNine' || region === 'localbusinessNine' || region === 'barberNineLeads' || region === 'localbusinessNineLeads' || region === 'localbusinessNineSpanish' || region === 'barberFiveNine') {
    ...
    const b9SingleTierFeats = b9IsSpanish ? [
      ...
      'Single page barbershop site',
      'Custom design + hosting + edits',
    ]);
    ...
    return (
      <>
        <style>{`
          ...
          .b9-sticky { position:fixed; left:8px; right:8px; bottom:8px; z-index:90; background:rgba(13,13,13,.97); backdrop-filter:blur(10px); -webkit-backdrop-filter:blur(10px); border-radius:10px; padding:4px 4px; box-shadow:0 8px 22px rgba(0,0,0,.5), 0 0 0 1px rgba(201,169,110,.40); max-width:360px; margin:0 auto; }
          .b9-sticky-toggle { display:flex; gap:2px; background:rgba(201,169,110,.10); padding:1px; border-radius:999px; margin:0 auto 3px; width:fit-content; }
          .b9-stoggle { background:transparent; border:0; padding:2px 8px; border-radius:999px; font-family:inherit; font-weight:700; font-size:8px; color:#cfc8b8; cursor:pointer; transition:background .15s ease, color .15s ease; display:inline-flex; align-items:center; gap:4px; }
          .b9-stoggle.active { background:#c9a96e; color:#0a0a0a; }
          .b9-stoggle .b9-savetag { background:#0a0a0a; color:#c9a96e; padding:1px 3px; border-radius:999px; font-size:7px; letter-spacing:.06em; font-weight:800; }
...
Keep replies short — 1 to 3 sentences usually. This is a phone call, not an essay.`,
  ...
  if (region === 'home' || region === 'denmark' || region === 'localbusiness29' || region === 'localbusiness9') {
    ...
    // /localbusiness-9-monthly = same no-hosting design copy, but a
    //   $9/mo single · $19/mo multi monthly subscription.
    const isDk = region === 'denmark';
    const isLb29 = region === 'localbusiness29';
    const isLb9 = region === 'localbusiness9';
    const isLbDesign = isLb29 || isLb9; // shared no-hosting / edits + custom-design copy
      ...
      <>
        ...
        <div className="mv-h-page">
          ...
          <div className="mv-h-stack">
            ...
            <section className="mv-h-card mv-h-hero mv-h-anim">
              <div className="mv-h-card-text">
                <div className="mv-h-eyebrow">{isLbDesign ? 'Custom local business websites · 48 hours' : t('Local business websites · 48 hours', 'Hjemmesider til lokale virksomheder · 48 timer')}</div>
            ...
            <section className="mv-h-card mv-h-how mv-h-anim">
              <div className="mv-h-card-text">
                <div className="mv-h-eyebrow">{t('How it works', 'Sådan fungerer det')}</div>
                <h2 className="mv-h-title">{t('Three simple steps.', 'Tre enkle trin.')}</h2>
                <p className="mv-h-sub">{t('All we need is a link. We handle the rest.', 'Vi skal bare bruge et link. Vi klarer resten.')}</p>
              </div>
            ...
            <section className="mv-h-card mv-h-pillars mv-h-anim">
              <div className="mv-h-card-text" style={{ width: '100%' }}>
                <div className="mv-h-eyebrow">{t('What you get', 'Hvad du får')}</div>
                <h2 className="mv-h-title">{isDk ? <>Bygget til at <em>hjælpe lokale virksomheder</em> med at vokse.</> : <>Built to <em>help local businesses</em> grow.</>}</h2>
                <div className="mv-h-pillar-grid">
                  <div className="mv-h-pillar">
                    ...
                    <div className="mv-h-pillar-h">{t('Trustworthy design', 'Troværdigt design')}</div>
                    <div className="mv-h-pillar-b">{t('A polished site can help customers take your local business seriously before the first call.', 'En gennemført hjemmeside kan hjælpe kunderne med at tage din lokale virksomhed seriøst allerede før første opkald.')}</div>
                  </div>
                  <div className="mv-h-pillar">
                    <svg className="mv-h-pillar-icon" viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                      <circle cx="24" cy="24" r="20" stroke="#0d0d0d" strokeWidth="2"/>
                    ...
                    <div className="mv-h-pillar-h">{t('Live in ~48 hours', 'Live på ~48 timer')}</div>
                    <div className="mv-h-pillar-b">{t('Share a bit about your business and we can have it online in about two days.', 'Fortæl os lidt om din virksomhed, så kan vi have den online på cirka to dage.')}</div>
                  </div>
                  <div className="mv-h-pillar">
                    <svg className="mv-h-pillar-icon" viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                    ...
                    <div className="mv-h-pillar-h">{t('SEO optimized', 'SEO-optimeret')}</div>
                    <div className="mv-h-pillar-b">{t('Clean code, schema markup, and fast loading — modern SEO best-practices baked in.', 'Ren kode, schema-markup og hurtig indlæsning — moderne SEO-bedste praksis er bygget ind.')}</div>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section id="mv-h-pricing" className="mv-h-card mv-h-pricing mv-h-anim mv-h-pricing-section">
              <div className="mv-h-pricing-head">
                ...
                <p className="mv-h-sub">{isLbDesign ? 'A custom design with edits included — either way. Pick the package that fits your business.' : t('$0 design fee either way. Pay monthly — covers hosting and ongoing edits.', '0 kr. i designgebyr uanset hvad. Betal månedligt — inkluderer hosting og løbende rettelser.')}</p>
              </div>
              ...
              <div className="mv-h-tier-row">
                <div className="mv-h-tier">
                  ...
                  <ul className="mv-h-tier-list">
                    <li>{isLbDesign ? 'One-page custom design' : t('One-page custom site', 'Skræddersyet hjemmeside på én side')}</li>
                    <li>{t('Mobile-responsive · SEO basics', 'Mobilvenlig · grundlæggende SEO')}</li>
                    <li>{t('Live in ~48 hours', 'Live på ~48 timer')}</li>
                    <li>{isLbDesign ? 'Edits included' : t('Hosting + edits included', 'Hosting + rettelser inkluderet')}</li>
                  </ul>
                ...
                <div className="mv-h-tier mv-h-tier-feat">
                  <div className="mv-h-tier-eyebrow">{isLbDesign ? 'Multi-page' : t('Multi-service', 'Flere ydelser')}</div>
                  <div className="mv-h-tier-price">{multiPrice}<span>{perMo}</span></div>
                  <ul className="mv-h-tier-list">
                    <li>{isLbDesign ? 'Multiple custom-designed pages' : t('Multiple service pages', 'Flere ydelsessider')}</li>
            ...
            <section className="mv-h-card mv-h-asseenon mv-h-anim">
              <div className="mv-h-card-text">
                <div className="mv-h-eyebrow">{t('Featured', 'Omtalt')}</div>
                <h2 className="mv-h-title">{isDk ? <>Set på Facebook, TikTok &amp; <em>Instagram.</em></> : <>As Seen On Facebook, TikTok &amp; <em>Instagram.</em></>}</h2>
                <p className="mv-h-sub">{t('Me personally walking into local businesses and building them sites.', 'Mig personligt, der besøger lokale virksomheder og bygger hjemmesider til dem.')}</p>
            ...
            <section className="mv-h-card mv-h-faq mv-h-anim">
              <div className="mv-h-card-text" style={{ width: '100%' }}>
                <div className="mv-h-eyebrow">{t('Questions', 'Spørgsmål')}</div>
                <h2 className="mv-h-title">{isDk ? <>Ofte stillede <em>spørgsmål.</em></> : <>Common <em>questions.</em></>}</h2>
                <div className="mv-h-faq-list">
                  <details className="mv-h-faq-item" open>
                    <summary className="mv-h-faq-summary">{t('What do I get with the website?', 'Hvad får jeg med hjemmesiden?')}<span className="mv-h-faq-icon">+</span></summary>
                  ...
                  <details className="mv-h-faq-item">
                    ...
                    <div className="mv-h-faq-a">{t('Email or SMS, anytime. Real humans on the other end.', 'E-mail eller SMS, når som helst. Rigtige mennesker i den anden ende.')}</div>
                  </details>
                  <details className="mv-h-faq-item">
                    <summary className="mv-h-faq-summary">{t('How fast is the website delivered?', 'Hvor hurtigt leveres hjemmesiden?')}<span className="mv-h-faq-icon">+</span></summary>
                    <div className="mv-h-faq-a">{t('Usually about 48 hours from sign-up to live URL.', 'Som regel cirka 48 timer fra tilmelding til live-URL.')}</div>
                  </details>
                </div>
              </div>
            </section>
            ...
            <section className="mv-h-card mv-h-final mv-h-anim">
              ...
              <p className="mv-h-sub">{isLbDesign ? `A custom design with edits included. From ${singlePrice}${perMo}.` : t('$0 design fee. From $20/month — covers hosting and edits.', '0 kr. i designgebyr. Fra 149 kr./måned — inkluderer hosting og rettelser.')}</p>
              <button className="mv-h-pill" onClick={() => { const el = document.getElementById('mv-h-pricing'); if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' }); }} disabled={isLoading}>
                {t('See pricing', 'Se priser')}
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
              </button>
          ...
          <div className="mv-h-sticky">
            <span className="mv-h-sticky-text">{isLbDesign ? `Custom design + edits · from ${singlePrice}${perMo}` : t('$0 design fee · from $20/mo', '0 kr. i designgebyr · fra 149 kr./md.')}</span>
            <button className="mv-h-pill" onClick={() => { const el = document.getElementById('mv-h-pricing'); if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' }); }} disabled={isLoading}>
              {t('See pricing', 'Se priser')}
            </button>
          </div>
